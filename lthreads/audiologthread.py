...
class AudioLogThread(threading.Thread):
    ...
    def run(self):
        while True:
            line1 = tailhead.tail(open('/var/log/mycroft/audio.log', "rb"), 3)
            line1sant = str(line1[0].decode("utf-8"))
            emit('my_response', {'data': line1sant})
            for line2 in tailhead.follow_path('/var/log/mycroft/audio.log'):
